...
class MixText(nn.Module):
    def __init__(self, num_labels=2):
        ...
        self.bert = BertModel4Mix.from_pretrained('./pretrain/')

        self.linear = nn.Sequential(nn.Linear(768, 128),
                                    nn.Tanh(),
                                    nn.Linear(128, num_labels))

    def forward(self, x, x2=None, lambda_l=None, mix_layer=1000):

        if x2 is not None:
            all_hidden, pooler = self.bert(x, x2, lambda_l, mix_layer)
